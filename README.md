...
“不羡仙”是一款计划长期开发的独立本地文字游戏。当前仓库已完成**工程基线、无界面确定性领域内核和首个版本化存档切片**；尚未实现可玩功能或叙事。
...
当前 HTTP 和浏览器层仍只有工程用途的 `GET /api/health` 接口及连接状态页面。Python 后端另有一个不接入 API 的纯领域内核，以及位于领域之外的版本化 JSON 存档和可恢复随机源适配器。它们目前仅使用中性合成状态验证工程契约。
